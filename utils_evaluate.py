...
def full_coref_evaluation(
    ...
    with open(log_file, "w") as f:
        ...
        dd = data_iter_map[split][dataset]
        for example in dd:
            start_time = time.time()
            pred_mentions, mention_scores, gt_actions, pred_actions = model(example, cache_mode=cache_mode)
            # Process predicted clusters
            raw_predicted_clusters, cor_cnt = action_sequences_to_clusters(
                pred_actions, pred_mentions, ask_cor=True
            )
            correct_cnt += cor_cnt
        ...
        for indv_metric, indv_evaluator in zip(config.metrics, evaluator.evaluators):
            perf_str += (
                ", " + indv_metric + ": {:.3f}".format(indv_evaluator.get_f1() * 100)
            )
            result_dict[indv_metric] = OrderedDict()
            result_dict[indv_metric]["recall"] = round(
                indv_evaluator.get_recall() * 100, 3
            )
            result_dict[indv_metric]["precision"] = round(
                indv_evaluator.get_precision() * 100, 3
            )
            result_dict[indv_metric]["fscore"] = round(indv_evaluator.get_f1() * 100, 3)

        result_dict["fscore"] = round(evaluator.get_f1() * 100, 3)
        logger.info("Cache %s Correct %d F-score: %.3f %s" % (cache_mode, correct_cnt, result_dict["fscore"], perf_str))
