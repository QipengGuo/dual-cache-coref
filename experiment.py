...
class Experiment:
    ...
    def __init__(self, config: DictConfig):
        ...
        self.max_ent = config.new_max_ent
        self.eval_ent = config.new_eval_ent
        ...
        if not self.eval_model:
            ...
            back = self.best_model_path
            self.best_model_path = "joint_best/model.pth"

            self._initialize_best_model()
            self.best_model_path = back
    ...
    def _build_model(self) -> None:
        ...
        self.model = EntityRankingModel(
            model_config=model_params, train_config=train_config, max_ent=self.max_ent, eval_ent=self.eval_ent, cache_mode=self.cache_mode
        )
    ...
    def _initialize_best_model(self):
        ...
        self.model = EntityRankingModel(config.model, config.trainer, max_ent=self.max_ent, eval_ent=self.eval_ent, cache_mode=self.cache_mode)
    ...
    def load_model(self, location: str, last_checkpoint=True) -> None:
        ...
        if self.config.model.doc_encoder.finetune:
            ...
            #doc_encoder_dir = self.config.paths.doc_encoder_dirname
            logger.info(
                "Loading document encoder from %s" % path.abspath(doc_encoder_dir)
            )
