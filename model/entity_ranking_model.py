...
class EntityRankingModel(nn.Module):
    ...
    def __init__(self, model_config: DictConfig, train_config: DictConfig, max_ent=None, eval_ent=None, cache_mode=None):
    ...
    def calculate_coref_loss(
        ...
        for idx, (cell_idx, action_str) in enumerate(action_tuple_list):
            ...
            if action_str == "c":
                ...
                continue
            if gt_idx>=len(action_prob_list[counter]):
                gt_idx = len(action_prob_list[counter])-1
            target = torch.tensor([gt_idx], device=self.device)
            coref_loss += self.loss_fn(
                torch.unsqueeze(action_prob_list[counter], dim=0), target
            )
    ...
    def forward_training(self, document: Dict) -> Dict:
        ...
        ment_scores = proposer_output_dict["ment_scores"],
        gt_actions: List[Tuple[int, str]] = get_gt_actions(
            pred_mentions_list, truncated_document_clusters, self.config.memory.mem_type, ment_scores, train=True, max_ent=self.max_ent, cache_mode=self.cache_mode
        )
        ...
        if self.mem_type == "unbounded":
            coref_new_list = self.memory_net.forward_training(
                pred_mentions, mention_emb_list, gt_actions, metadata, ment_scores=ment_scores, MAX_ENT=self.max_ent, cache_mode=self.cache_mode
            )
        else:
            ...
            if len(new_ignore_list) and False:
    ...
    def forward(self, document: Dict, MAX_ENT=600, cache_mode=None) -> Tuple[List, List, List, List]:
        ...
        #print(self.config.memory)
        pred_mentions_list, mention_scores, pred_actions, pred_actions1 = [], [], [], []
        ...
        for idx in range(0, len(document["sentences"])):
            ...
            cur_pred_actions, entity_cluster_states =  self.memory_net(
                ...
                proposer_output_dict["ment_scores"],
                cache_mode,
            )


            pred_actions.extend(cur_pred_actions[:len(cur_pred_mentions)])
            pred_actions1.extend(cur_pred_actions[len(cur_pred_mentions):])

        gt_actions = get_gt_actions(
            pred_mentions_list, document, self.config.memory.mem_type, ment_scores=mention_scores
        )  # Useful for oracle calcs
        return pred_mentions_list, mention_scores, gt_actions, pred_actions+pred_actions1
